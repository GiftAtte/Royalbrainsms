...
<div class="content-header">
 ...
 <div class="card card-navy card-outline">
  ...
  <h4 class=" ml-5"> {{student.surname}} {{student.first_name}}   ------------  {{student.levels.level_name}} </h4>
  <h4 class=" ml-5 text-danger">Score: {{ score+'/'+questions.length}}   =====>  ({{Number.parseFloat((score/questions.length)*100).toPrecision(4)+'%'}})</h4>
               <hr>
                <div v-for="(question,index) in questions" :key="question.id" class="col pb-1 ml-3" >
                    <div class="row">
                    <span>{{index+1}}.</span> <p v-html="question.question.question">  </p>
                    <i class="fa fa-times red ml-5" aria-hidden="true" title="wrong answer" v-show="!question.question.is_right"></i>
                   <i class="fa fa-check green ml-5" aria-hidden="true" title="right answer" v-show="question.question.is_right" ></i>
                    </div>
        ...
        methods: {
         ...
         loadQuestions(){
                ...
                if(this.$gate.isAdminOrTutorOrStudent()){
                    axios.get(`/api/review_answers/${this.$route.params.exam_id}/${this.$route.params.student_id?this.$route.params.student_id:''}`)
