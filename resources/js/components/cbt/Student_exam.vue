                            ...
                            <tbody>
                                ...
                                <tr v-for="exam in exams.data" :key="exam.id">
                                    ...
                                    <td class="action">
                                        ...
                                        <a
                                            href="#"
                                            @click="newModal(exam.id)"
                                            v-show="$gate.isAdminOrTutor()"
                                            :to="`/cbt_scores/${exam.id}`"
                                            class="m-1 btn btn btn-sm btn-success"
                                        >
                                            Use
                                        </a>
                    ...
                    <form @submit.prevent="editmode ? updateExam() : useCBT()">
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                <input
                                    type="number"
                                    v-model="form.cbt_value"
                                    class="form-control"
                                    id="title"
                                    placeholder=" A percentage of : eg  10, 15, 30"
                                    :class="{
...
export default {
    ...
    methods: {
        getResults(page = 1) {
            axios.get("/api/publishedExams?page=" + page).then((response) => {
        ...
        startExam(exam) {
            swal.fire({
                title: "Instruction!!!",
                html: `<p>${
                    exam.comment ? exam.comment : ""
                }<br/><b> Press the continue button to start. <br/>Your time starts counting immediatetly </b></p>`,
                type: "warning",
        ...
        useCBT() {
            if (this.$gate.isAdminOrTutor()) {
                this.form.post("/api/use_cbt").then((res) => {
