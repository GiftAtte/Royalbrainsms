...
export default {
    data() {
        return {
            ...
            feeTypes: [
                ...
                {
                    text: "New Intakes-Class specific",
                    value: "NEW-CLASS-BASED",
                },
    ...
    methods: {
        getResults(page = 1) {
            axios.get("api/fees?page=" + page).then((response) => {
        ...
        checkFeeType() {
            if (
                this.form.fee_type === "CLASS-BASED" ||
                this.form.fee_type === "NEW-CLASS-BASED"
            ) {
        ...
        loadPaystack() {
            if (this.$gate.isAdmin()) {
                axios
                    .get("/api/paystack")
                    .then((res) => (this.payStack = res.data));
            }
        },
        AddDetails(id) {
        ...
        createDescription() {
            ...
            this.DetailsForm.post("api/fee_description")
                .then((res) => {
                    ...
                    this.$Progress.finish();
                    this.$router.push(
                        "/fee_description/" + res.data.feegroup_id
                    );
                })
                .catch(() => {});
        },
